...
- Use a [`time_date` sensor ](/components/sensor.time_date/) and a template .
...
Hmmm, ...OK, that's a start. How to get the year? . This means that you can access more than you think in a template: 
...
For the year it would be: . I guess that there are rare use cases for `now().resolution`, `now().min` and `now().max` too.
