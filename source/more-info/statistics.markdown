...
Make sure the entity uses the same unit as the already selected sensors for that statistics graph. The statistics card uses one Y axis, so mixing units (for example, °C and % or kW and W) is not supported in the same graph.
