...
If the disk is disconnected, or it is missing when your system starts, Home Assistant creates a repair issue at **{% my repairs title="Settings > System > Repairs" %}**. Reconnect the disk, then reload the storage from **{% my storage title="Settings > System > Storage" %}**. If you no longer want to use the disk, select the storage and select **Delete** instead.
