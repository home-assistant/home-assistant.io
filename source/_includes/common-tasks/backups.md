...
When downloading the backup from the backup page in Home Assistant, the backup is decrypted on the fly so that you then can view the data using your favorite archive tool. This is done for all backup locations, also when you download from Home Assistant Cloud.
