...
The database file is stored in your [configuration directory](/docs/configuration/#to-find-the-configuration-directory) (for example, `<path to config dir>/home-assistant_v2.db`); however, other databases can be used. If you prefer to run a database server such as PostgreSQL, use the [`recorder`](/integrations/recorder/) integration.
