...
TOTP is _time based_ so it relies on your Home Assistant clock being accurate. If the verification keeps failing, make sure the clock on Home Assistant is correct. In orther the clock to be accured Home Assistant needs to have internet access or local [NTP server](https://en.wikipedia.org/wiki/Network_Time_Protocol) available. If you will enable TOTP when no internet access is available and you are locked out from your account, to restore access you would need to remove or rename `auth_module.totp` file and restart your Home Assistant instance. The file is typically stored in `/supervisor/homeassistant/.storage` folder. You can do that by using previously setup Samba share add-on or login via SSH to you Home Assistant ([Home Assistant OS SSH login](https://developers.home-assistant.io/docs/operating-system/debugging/#home-assistant-operating-system)).
