...
Home Assistant will trigger an event when the Z-Wave network is complete, but some node ar mark dead, meaning all of the nodes on the network have been queried. This can take quite some time, depending on wakeup intervals on the battery-powered devices on the network.
