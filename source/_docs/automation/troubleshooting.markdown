...
When an {% term automation %} is run, all steps are recorded and a trace is made. From the UI, choose **Settings** which is located in the sidebar, then click on **Automations & Scenes** to go to the automation editor or click this button directly: {% my automations badge %}
