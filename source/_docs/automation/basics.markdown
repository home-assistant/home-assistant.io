...
Automations interact directly with the internal state of Home Assistant, so you'll need to familiarize yourself with it. Home Assistant exposes its current state via **Tools**. These are available at the bottom of the sidebar in the frontend. {% my developer_states title="**Settings** > **Tools** > **States**" %} will show all currently available states. An entity can be anything. A light, a switch, a person and even the sun. A state consists of the following parts:
