...
- `area_temperature(area_name_or_id)` returns the temperature indicated by the temperature sensor assigned to a given area identified by ID or name. Can also be used as a filter.
- `area_humidity(area_name_or_id)` returns the humidity indicated by the humidity sensor assigned to a given area identified by ID or name. Can also be used as a filter.
...
```text
{{ area_temperature('Living Room') }}  # 21.3
```

```text
{{ area_humidity('Living Room') }}  # 48
```
