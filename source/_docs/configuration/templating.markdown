...
- `timestamp_as_datetime()` converts a UNIX timestamp to a datetime object.

  {% raw %}

  ```yaml
  # Convert a UNIX timestamp to a datetime object and convert it to local time
  {{ as_timestamp(now()) | timestamp_as_datetime | as_local }}
  ```
