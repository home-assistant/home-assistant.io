...
- `wrap(v, min, max)` limits the value to be between min and max, wrapping the value at the edges; in math speak, that's [modular arithmetic](https://en.wikipedia.org/wiki/Modular_arithmetic), and in common terms, "clock face math".
