...
One of the most common problems with Home Assistant is an invalid `configuration.yaml` file. 
 ...
 - You can test your configuration using the command line with: `hass --script check_config`. On Hass.io you can use the [hassio command](/hassio/commandline/#home-assistant): `hassio homeassistant check`. Also, you can run `python3 -m homeassistant --script check_config` from the command line when you have a shell prompt at `/usr/src/app`
