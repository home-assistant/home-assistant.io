...
* To ensure that homeassistant displays the correct timezone go to the "Environment" tab & click the plus sign then add `variable` = `TZ` & `value` = `Europe/London` choosing [your correct timezone](http://en.wikipedia.org/wiki/List_of_tz_database_time_zones).
