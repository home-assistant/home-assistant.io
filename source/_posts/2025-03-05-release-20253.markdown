...
- [Assist now has streaming responses](#assist-now-has-streaming-responses)
...
card Home Assistant current has. But ever since its creation, it had a little
...
when it does have an actions, it will show the circle, meaning you can tap that
little button.
...
it, which, well at times could be helpful I guess.

But what if we could do something useful with that space? Now you can!
...
This brings a completely new look and feel option to the tile car, of which we
are sure, you are going to love! From the setting perspective, you can thus
set the position of the feature to be either vertically or on the right side.
...
For example, we now have nice tiny little animations when you however over
...
## Assist now has streaming responses

If you have hooked up an LLM, like ChatGPT, as an LLM to your Assist as
a conversation agent, it will now live stream the responses to you when
...
- Add support for effects in Govee lights ([@Galorhallen] - [#137846]) govee_light_local
...
- Every upgraded Home Assistant and needed to do a hard refresh in your
  browser to make it work again? That is now a thing of the past! Every
  time your browser connects to Home Assistant, it will ensure you browser's
  cache is up-to-date. Thanks, [@bramkragten]!
- [@balloob] make a small change with a big impact. When you set up an
  new integration for a new device, Home Assistant will now redirect you
  to the device page after setting it up. Nice!
- There is now an option, to add extra margin to the top of a section view.
  Thanks, [@piitaya]!
- The add/edit area dialog has been improved and compacted. They are
  much more plesant to use now. Thanks, [@jpbede]!
...
- Media player entities supporting browing media, now have an new action
  available to [browse media as an action with a response]. Thanks, [@PeteRager]!
- [@rikroe] ha added a action to [retrieve the configuration of a schedule
  helper]. Thanks!
...
but or aim was to make it look and feel similar.

However, we received a lot of feedback from the community about the legends
shown on the new graphs, being suboptimal. Our UX and frontend teams have
...
as possible. If there are to many, they will be displayed on demand using the
little elipsis button.

When using these cards on the dashboard, and you prefer them to always we
...
Some programs at program switches (which are being deprecated) and program
select entities may not be exposed anymore by the integration as they are
missing at `aiohomeconnect` program key enumeration ([file](https://github.com/MartinHjelmare/aiohomeconnect/blob/ccfbf802884aa781eef481da92e6bccd2e6de733/src/aiohomeconnect/model/program.py#L213C1-L507C6)).
...
Using program and option keys that aren't at the API documentation, will no
...
Users with a YAML setup have been instructed to update there configuration in
...
the logs. Use of the removed parameters in discovery wil no break
the configuration, but will still log warning.
...
switch, since they never really worked, because there were no energy or power
capabilities available to get data from. If you have the capabilities,
