...
Managing who can unlock your Z-Wave lock just got a lot more capable. Powered by Z-Wave JS's new Access Control API, the Z-Wave JS add-on 1.6.0 brings a heavily revised interface: every supported lock now gets a dedicated **Access Control** tab. For now, this lives in Z-Wave JS UI rather than in the Z-Wave panel in Home Assistant.
...
Update the Z-Wave JS add-on to 1.6.0 or later, then open Z-Wave JS UI to find the new tab. Thanks, [@AlCalzone]!
...
<!-- Breaking-changes sweep checkpoint: swept through release 2026.8.0b3. -->
