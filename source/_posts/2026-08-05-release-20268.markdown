...
Managing who can unlock your Z-Wave lock just got a lot more capable. Powered by Z-Wave JS's new Access Control API, Z-Wave JS UI 1.6.0 brings a heavily revised interface: every supported lock now gets a dedicated **Access Control** tab. For now, this lives in Z-Wave JS UI rather than in the Z-Wave panel in Home Assistant.
...
Update Z-Wave JS UI to 1.6.0 or later, then open it to find the new tab. Thanks, [@AlCalzone]!
