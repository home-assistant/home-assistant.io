...
- **[UniFi Protect]** now supports the USL smart sensors (USL Entry, USL Environmental, and USL GlassBreak) on UniFi Protect 7.2 and newer, and cameras gained **Motion detection**, **Smart detection**, and **Sound detection** event entities that fire for any detection the camera reports, such as a person, a package, or barking, with the detected type as an attribute. The UP Air Quality sensor is not supported yet. Thanks, [@RaHehl]!
