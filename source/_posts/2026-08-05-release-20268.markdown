...
I'm [Ashley](https://github.com/abb848), and with [Frenck](https://github.com/frenck) taking a well-deserved break, I'm teaming up with [Bram](https://github.com/bramkragten) to handle the release this time around.

The team and I have been working on a lot over the past few weeks, and I'm very excited to share some of what we have done for this release!
...
<!-- REMOVE before final: covered elsewhere, so intentionally left out above — Onboarding survey (#53236, #53221, @maartenla) is under "Tell us how setup went"; Redesign cloud page (#53025, @klejejs) is under the Home Assistant Cloud section. -->
...
**TODO: REMOVE BEFORE FINAL RELEASE**: Backward-incompatible changes processed up until PR #177187

<!-- Sweep resolved: Device registry (#175785), Gardena Bluetooth (#177187), Paperless-ngx (#176889), and Volvo On Call (#175677) are all written below. Steam #176146 is NOT a breaking change (new feature: it adds sensors), so it moved to the noteworthy improvements list. Developer-facing only and intentionally excluded from this user list: #175606, #175608, #175611 (all @gjohansson-ST). -->

{% details "Device registry" %}
...
There is nothing you need to do. Your automations and scripts keep working, because Home Assistant still sends actions to the right devices after the split. The only things that might need a second look are automations that reference a device by its internal ID directly, or the occasional unusual setup. If a repair appears, just follow it.
...
If you develop a {% term "custom integration" %}, most do not work with the device registry directly and need no changes. If yours does, the [developer blog](https://developers.home-assistant.io/blog/2026/07/21/device-registry-single-config-entry) covers what changed. The previous methods keep working for a twelve-month period, and this release ships without deprecation warnings on purpose, so there is nothing to rush.

([@emontnemery](https://github.com/emontnemery) - [#175785](https://github.com/home-assistant/core/pull/175785))
