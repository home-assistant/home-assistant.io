...
- The statistics component received a bunch of new statistical characteristics
  and one can be chosen to be presented as the state of the sensor directly.
  For performance reasons the full set of characteristcs was removed from
  attributes ([@ThomDietrich]).
...
directly.
The parameter `state_characteristic` will become mandatory for configuration
in a future release.

Users will need to adapt their configuration and expose the desired characteristic.
Additional template sensors to expose attributes are no longer necessary and
existing ones will no longer work.

Additionally, many more characteristics were added, including a few for a
binary source sensor. Most characteristics were renamed. Please check
documentation.
