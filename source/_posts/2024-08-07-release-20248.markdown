...
- [Dashboards - Chapter 2](#dashboards---chapter-2)
  ...
  - [Picture cards get image from the person entity](#picture-cards-get-image-from-the-person-entity)
...
### Picture cards get image from the person entity

Remember when changing a person picture and picture cards did not reflect the brand new image ?

That's now an old story, picture cards can now fetch your person picture, so you don't have to update them manually !

__Example on `picture-glance` card__

- Before: it uses a static image

```yaml
type: picture-glance
entity: person.paulus
image: /api/image/serve/a52be54b0cefa35bf27abac3b2a3df1a/512x512
entities:
 - weather.paris
```

- After: it retrieves the image from the person entity automatically
...
The result is the same, until you change Paulus's picture and you see that with the `image` attribute, the card is keeping the previous picture.
