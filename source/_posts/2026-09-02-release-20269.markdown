...
The first integration built this way is already here: **[Fronius]** gained optional Modbus TCP (SunSpec) support, adding per-string solar data, current, voltage, power, and lifetime energy for each MPP tracker, that its existing cloud API doesn't expose. It shares its connection with anything else talking to the same inverter, which matters in practice: some Fronius models only accept a handful of simultaneous Modbus sessions.
...
Thanks to Paulus ([@balloob](https://github.com/balloob)) for pushing this forward, [@wlcrs](https://github.com/wlcrs) for tmodbus, the library it's built on, and [@farmio](https://github.com/farmio) for Fronius!
...
- **[Template]** now lets you add custom attribute templates to alarm control panel, button, cover, device tracker, fan, image, light, lock, number, select, switch, update, and weather entities, alongside the sensor and binary sensor platforms that already supported them. Thanks, [@Petro31]!
