...
description: "Home Assistant Cloud gets clearer login visibility and a friendlier signup flow, the Security dashboard gains active alerts and favorites, Activity now explains why something changed, Matter gets its own network map, Modbus integrations learn to share a connection, charts become accessible by keyboard and sound, and 13 new integrations!"
...
categories:
  - Release-Notes
  - Core
og_image: /images/blog/2026-09/social.png
---
...
I'll be honest: Modbus has never been the flashiest part of Home Assistant. It quietly powers solar inverters, heat pumps, and energy meters, but only if you're comfortable hand-writing your own register map in YAML, a real wall that's kept a huge category of devices out of reach. This release finally makes room on the bus: integrations that already know how a device talks over Modbus, sharing a single connection instead of fighting over it, so you simply pick your device in the UI like anything else in Home Assistant. It won't show up in a screenshot, but [modernizing Modbus](#modernizing-modbus) is hands-down my favorite change this release. 🔧

I'm also really looking forward to jumping on [the new Cloud voice processing](#test-the-new-voice-processing) myself and putting it through its paces. We're a household with a non-English main language, and if you've ever run a private voice assistant in one, you know exactly how fast accents and background noise trip things up. This new speech-to-text engine specifically targets those pain points, so I'm very curious to see how it holds up against my own accent. 😉

And there's plenty more: [active alerts and favorites](#active-alerts-and-favorites-for-the-security-dashboard) for the Security dashboard, a shared [Sources panel](#a-new-sources-pane-for-history-and-activity) for History and Activity, Activity that finally explains [why something changed](#from-what-changed-to-why-it-changed), a brand new [network map for Matter](#insights-into-your-matter-world), charts you can now [navigate by keyboard and sound](#accessibility-for-charts), and 13 new integrations! 🚀

One more thing before you dive in: we're at [IFA Berlin](https://www.openhomefoundation.org/blog/ifa-berlin-2026-save-the-date/) this weekend! It's one of the largest consumer electronics expos out there, running September 4 to 8, 2026, and the Open Home Foundation has its own [booth in the Smart Home hall](https://www.ifa-berlin.com/exhibitors/open-home-foundation): Hall 1.2, stand 1.2-153. If you're around, come find us and say hi. 👋
...
- [Home Assistant Cloud](#home-assistant-cloud)
  - [Seeing through the Cloud](#seeing-through-the-cloud)
  - [Test the new voice processing](#test-the-new-voice-processing)
  - [Float in to be a Cloud subscriber](#float-in-to-be-a-cloud-subscriber)
- [Active alerts and favorites for the Security dashboard](#active-alerts-and-favorites-for-the-security-dashboard)
...
- [Patch releases](#patch-releases)
  - [2026.9.1 - September 5](#202691---september-5)
  - [2026.9.2 - September 11](#202692---september-11)
  - [2026.9.3 - September 18](#202693---september-18)
  - [2026.9.4 - September 25](#202694---september-25)
...
[@jan-tdy]: https://github.com/jan-tdy
[@piitaya]: https://github.com/piitaya
[@googanhiem]: https://github.com/googanhiem

## Home Assistant Cloud

[Home Assistant Cloud](https://www.home-assistant.io/cloud) is an essential part of funding the [Open Home Foundation](https://www.openhomefoundation.org/). We're so thankful for all the support from our subscribers, and from the start we've always been clear: this isn't just a donation to Home Assistant, we want it to provide real benefits to subscribers. Together with [Nabu Casa](https://www.nabucasa.com/), we've been constantly working to make Cloud better, and here are some useful recent additions.

### Seeing through the Cloud

A new addition that should help security-conscious [Home Assistant Cloud](https://www.home-assistant.io/cloud) subscribers is the ability to better see who is attempting to log into your instance. In the past, they all looked like local requests, and you may have seen the *127.0.0.1 invalid login attempt* message, which wasn't that helpful. Now the proper IP address gets passed through, so whether that's you mixing up your passwords when logging in, or some bot trying their luck, you'll now know better where it's coming from.

A side benefit of this change is that it's now possible to use IP banning to make it harder for these malicious actors, which was [added to the UI](https://www.home-assistant.io/blog/2026/08/05/release-20268/#want-to-change-the-port-weve-got-you-covered) in the previous release. In conjunction with setting up [multi-factor authentication](https://www.home-assistant.io/docs/authentication/multi-factor-auth/) (2FA) in Home Assistant, you can add a little extra peace of mind to your home, but that's not the only Cloud benefit being improved this month.

### Test the new voice processing

When we launched [Home Assistant Voice Preview Edition](https://www.home-assistant.io/voice-pe/), a good amount of the community jumped into running their own private voice assistants in their homes. Building something to rival big tech is still a [work-in-progress](https://www.home-assistant.io/blog/categories/assist/), but every year we get closer. Today, we're again asking for our Cloud subscribers to help us test another awesome step forward.

Our friends at [Nabu Casa](https://www.nabucasa.com/) are testing a new speech-to-text engine for Home Assistant Cloud, and it significantly improves the three common places voice processing gets tripped up: accents, background noise, and non-English languages. Many of us around the Foundation have been using it and finding it works great with our sometimes *unique* accents even in noisy environments.

There are several big changes under the hood, including switching to a new provider [Soniox](https://soniox.com/), so it needs to be tested. You'll still get the same privacy guarantee of no logging, storing, or training on your audio. It'll be available to test via [Labs](https://my.home-assistant.io/redirect/labs/), with the normal caveat that it may not be a permanent addition, so provide [your feedback](https://docs.google.com/forms/d/e/1FAIpQLSdIxGYIGDi1z71TH4m7KF9VVjeDgKfZ87WQsljZYtMPiizVmA/closedform) on the Labs page if it's the right step forward.

If you're a Home Assistant Cloud subscriber, you can toggle this on right now by heading to [**Settings \> System \> Labs**](https://my.home-assistant.io/redirect/labs/). If you're not a subscriber, there is a 31-day trial. Speaking of which, there are some big improvements on signing up coming as well.

### Float in to be a Cloud subscriber

[Signing up for Home Assistant Cloud](https://my.home-assistant.io/redirect/cloud/) now begins on a clearer start page: one obvious button to **Start your free trial,** and a link to **Sign in** if you already have an account. It also finally shows what you would actually be getting. The old page left you to guess what was behind that button, which, fair enough, didn't help anyone. Now the features are listed right there, before you even start the trial.
...
Now confirming your email will automatically log you in to Home Assistant Cloud, so you can immediately set up remote access, voice, and {% term backups %}. No more logging in after signing up! And as always, the one-month trial requires no payment details.
...
- **[Ridder HortiMaX Pro]**, added by [@wildekek]  
  Bring measurements from a [Ridder](https://www.ridder.com/) HortiMaX Pro greenhouse controller into Home Assistant, including temperatures, humidity, vent and screen positions, irrigation volumes, and energy use. The integration is read-only, so your growing decisions stay fully in the controller's hands. It's the same kind of controller running the greenhouses at [Amsterdam's Hortus Botanicus](https://www.dehortus.nl/en/), one of the oldest botanical gardens in the world, which we visited earlier Read [the full story](https://newsletter.openhomefoundation.org/the-walled-garden-thats-breaking-down-barriers/) in our newsletter.
...
Rebuilding {% term automations %} around the sun has been a slow-burn project, and this release wraps it up. [2026.7](/blog/2026/07/01/release-20267/) replaced the old, offset-heavy sun {% term trigger %} with dedicated ones for sunrise, sunset, solar noon, solar midnight, dawn, dusk, and elevation changes, plus matching {% term conditions %} like *is up*, *is night*, and *twilight*. [2026.8](/blog/2026/08/05/release-20268/) then added a time-based offset option to those triggers, the same way the calendar trigger already works, so you can fire an automation a set amount of time before or after a sun event.
...
<lite-youtube videoid="ox2lxlqeYAo" videotitle="Home Assistant 2026.9: Accessible charts, navigated by keyboard and sound"></lite-youtube>
