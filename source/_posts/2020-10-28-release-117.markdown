...
restarting Home Assistant, but expect this to grow rapidly grow!
...
We'd love to hear you experience with using this feature!
...
release. Visual tweaks, quality of live improvements and random fixes, like:
...
- You can save now automations, scripts and scenes with `ctrl`/`cmd` + `s`.
...
- We improved compatibility and Home Assistant is now usable on more, mostly
  older, devices.
- You can now search for the entity name in the entity picked besides the
...
- [@amelchio] also added support for playing Spotify URIs with the Sonos
  integration.
- Thanks to [@timmo001], the WLED integration now supports color pallets.
