...
update template entities that do not listen for state changes when `now()` or
`utcnow()` is present in the template.
...
- Remove BOM integration because it uses webscraping ([@balloob] - [#41941]) ([bom docs])
