...
Running Home Assistant on a Raspberry Pi? You are in good company: per our public, opt-in [analytics](https://analytics.home-assistant.io/), the Raspberry Pi platform currently powers about a third of all installations. As of this release, there is an {% term update "update entity" %} for the Pi's bootloader firmware, also often referred to as the <abbr title="electrically erasable programmable read-only memory">EEPROM</abbr> update. You can keep it current right from {% my updates title="**Settings** > **Updates**" %}, the same place you handle every other update.
