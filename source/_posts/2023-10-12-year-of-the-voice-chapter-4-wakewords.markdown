...
There are two downsides to this approach. The first is that the quality of the captured audio differs. A speakerphone with multiple microphones and audio processing chips captures voice very cleanly. A device with a single microphone and no post-processing? Not so much. It’s possible to compensate for poor audio quality with better speech-to-text models like the one included with Home Assistant Cloud. Additionally, we’ve included some audio post-processing inside Home Assistant like noise suppression and automatic gain control.
...
_(Wyoming is our protocol allowing to run parts of a voice assistant in other programs and/or computers)_
...
David created a Google Collab notebook to [create your own openWakeWord model.][own-wake-word] Enter your desired wake word and an hour later you get your own wake word (using the free computing available to all Google Collab users). This currently only works for English wake words because of the available Piper models: a voice model with many different speakers is needed. Similar models for other languages will be trained as data becomes available.

<insert video?>

Custom wake word models can be copied into the `/share/openwakeword` directory of your Home Assistant server, and will be instantly available to select when editing a voice assistant. If you’re running the Docker image separately, use `--custom-model-dir` to set your directory for custom wake words.
