...
Thanks to our community for keeping pace with the new integrations and
...
It is not just new integrations that are added; existing integrations
are also constantly improved. Here are some of the noteworthy changes
...
- Limited templates can now be used to disable triggers, conditions
  and actions. [@farmio] thought this might be helpful for blueprint creators.
...
The following integrations reached the platinum 🏆 level on our
...
interface, some were only available using YAML configuration. We keep moving
