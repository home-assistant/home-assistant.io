...
With this new setting, LLM-based conversation agents can tap into our intent system, which is what powers Assist. They also get access to every entity exposed to Assist. That way, you can control what you give access to your assistants.

Tapping into our intent system is great for two reasons.
First, it works out of the box. Everything that Assist can do, LLM-based conversation agents can too. The added benefit is that they are capable of reasoning beyond words, which is something Assist was not capable of doing.
For example, if you have a light called "Webcam light" exposed in your "Office" area, you can give direct commands such as "Turn on the office webcam light" (that was also working with Assist), but you can also give more complex commands such as "I'm going in a meeting, can you please make sure people see my face?". The agent will figure out the intention behind the words, and call the correct intent on the correct entities.

Second, our intent system has been built from the start to be extensible. custom integration and users can register their own intents. This allows you to guide the agent beyond what's capable out of the box.
Here is a simple example of a custom intent that explains to the LLM what to do when I leave my home. 
...
With this simple YAML snippet added to my configuration.yaml, if the agent understands that I am leaving home, it will not turn off what it thinks I want, it will run my `leave_home` script, which is exactly what I want it to do.
