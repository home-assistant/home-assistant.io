...
The IBM Watson IOT Platform, Neato and Plum Lightpad integrations are also no longer available as of this release. Their services have been discontinued. 

The Vultr integration has been removed. It relied on Vultr’s phased out API, which is no longer available. We’d be happy to see this integration return if someone updates it to work with Vultr’s newer API versions.
...
This release ships with Python π! (See what they did there? Python 3.14 → π 😉) 
If you’re wondering—Python is the language Home Assistant runs on. 

Why’s that exciting? It means a faster, more efficient foundation for everything Home Assistant does. 

And the best part? If you’re using Home Assistant OS or the Container version, you don’t have to do a thing. The upgrade to Python 3.14 happens automatically when you update Home Assistant.

Big thanks to [@cdce8p] for making it happen!
...
{% details "Config log file changes on supervised install" %}
...
{% details "Remove deprecated state constants" %}

Deprecated state constants in the `camera`, `cover`, `lock`, `media_player` and `vacuum` integrations have been removed. Update any custom components or automations using these constants.
...
The discovery of Renault functionnality was previously based on assumptions that may have created invalid and non-functionnal entities.
