...
- **Tile**
  ...
  This is now available to be configured from the Integrations UI. The entity IDs have changed to reflect the human-friendly name of the Tile (as defined in the Tile mobile app). Please update your automations/etc. accordingly.
  
  ([@bachya] - [#36173]) ([tile docs]) 

- **Panel_Custom**
  
  Specifying custom panels based on HTML imports is being deprecated and will be removed in a future version.
...
- **Avri**
  ...
  This is now available to be configured from the Integrations UI. Remove the integration from configuration.yaml and re-add it in the UI.
  
  ([@timvancann] - [#34288]) ([avri docs]) 

- **Arcam**
  
  This is now available to be configured from the Integrations UI. Remove the integration fromconfiguration.yaml and re-add it in the UI. To support turn_on action, you will need to setup an automation based on an exposed event from the integration. This can be accomplished via device actions.
...
- **Xiaomi Miio**
  ...
  These all now requirethat either `entity_id` or `area_id` are passed when calling the service. You need to update all usages (such as automations and scripts) of these services which do not already pass `entity_id` or `area_id`. For example, change the existing automation:
...
- **Homematic IP Cloud
  ...
  The HMIP-MOD-TM devices will get a new unique_id due to a merge of the implementation with new HMIP-MOD-HO. So check your dashboards, scripts, automation, ... and use the newly created device/entity. The old device can be deleted under the device section in Home Assistant.
...
- **Yeelight**
  ...
  It is no longer be possible to override `device_type` via model from config, to force some supported features. For example force ceiling light into generic light etc. Now it will only be used for yeelight lib, to get correct color temp ranges. It still possible to pass declared model to yeelight lib to get correct temp ranges support.
...
- **Denon AVR Network Receivers**
  ...
  This is now available to be configured from the Integrations UI. Please remove the denonavr platform from your configuration.yaml file. Denon and Marantz receivers should be discovered automatically and show up under "Configuration"-> "Integrations", from there click on configure and go through the steps. All previous setting options are still available during this setup.
  If your receiver does not show up automatically, go to "Configuration"-> "Integrations"-> press the "+" icon -> search for "denonavr" and enter the setup.

  ([@starkillerOG] - [#35255]) ([denonavr docs])

- **Smapee**
  
  The Smappee integration has been completely renewed with dev API v3 to support all types of Smappee monitors (Energy, Solar, Pro, Plus, Genius, Connect, P1S1 module) and switches (Comfort plug, Switch). Existing old entities will still be available in this new integration, but their unique IDs will have changed. Users will need to remove the old remaining entries from the entity registry.
  
  The configuration.yaml file now only requires the `client_id` and `client_secret`. The username and password will be asked with the new oauth2 helper redirect. Please check the documentation for the details how to set up the integration.
...
- **Awair**
  ...
  Additionally, the Awair integration no longer sets custom "device_class" attributes for non-standard sensors (such as the "Awair Score" sensor). This does not affect the operation of the sensors, but if you relied on these non-standard values in your configuration you may need to make adjustments.Refactor / update Awair integration
...
- **Xiaomi Gateway (Aqara)**
  ...
  Xiaomi Aqara Gateways schould be discovered automatically and show up under "Configuration"-> "Integrations", from there click on configure and go through the steps.
  If your Xiaomi Aqara Gateway does not show up automatically, go to "Configuration"-> "Integrations"-> press the "+" icon -> search for "xiaomi_aqara" and enter the setup. If no key is provided during setup, only the binary_sensor and sensor platforms will be available. A key is required to activate the other platforms.Add Xiaomi Aqara Config Flow
...
- **MQTT**
  ...
  - Remove MQTT broker `tls_version` parameter and instead rely on auto negotiation.
...
- Lockitron (platform is decommissioned) ([@frenck] - [#37012])
