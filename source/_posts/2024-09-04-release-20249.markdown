...
Instead of a display mode, you can decide yourself what element you want to show. This allows you for example to hide the icon of an badge and only show the state of the device you want to show in your badge.
...
- **[Yale]**, added by [@bdraco]<br>
  The Yale Home brand now has it's own integration and will not longer be using the [August] integration. If you use a Yale Home device with the August integration, you should migrate to the new Yale integration.
...
- The context window size of [Ollama](/integrations/ollama) is bigger and configurable now, making it perform 20% better! Great [@allenporter]!
- [@allenporter] Added event entities to the [Nest](integrations/nest) integation.
...
- [MotionBlinds](/integrations/motion_blinds) not have buttons for the favorite position. Thanks [@starkillerOG].
...
Not all migrations where successful despite being marked as successful. This 
...
The YAML configuration for the Ecovacs integration that was moved to the UI in Home Assistant 2024.2 has been removed.

The YAML configuration was automatically imported and should be removed your YAML configuration.
...
The YAML configuration for the GPSD integration that was moved to the UI in Home Assistant 2024.2 has been removed.
...
The YAML configuration for the Lupusec integration that was moved to the UI in Home Assistant 2024.2 has been removed.
...
{% details "Overkiz" %}
 ...
 A binary sensor was added to indicate if the manual mode is action.
...
Instead of switch a horn button entity was added. Please update your automations accordingly.
...
The YAML configuration for the Velux integration that was moved to the UI in Home Assistant 2024.2 has been removed.
...
- Firmware update entity logic has changed. A device will only be considered "up-to-date" if there is a known firmware update for it and the current firmware version is greater. If there is no known firmware update, the device firmware state will be considered "unknown", as we can't tell if it's up to date or not.
...
- [https://developers.home-assistant.io/blog/2024/09/02/template-hass-required](https://developers.home-assistant.io/blog/2024/09/02/template-hass-required)
