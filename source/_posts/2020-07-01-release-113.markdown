...
It seems like [@bdraco] is unstoppable, he just keep going on improving
...
Before diving in: All automation changes have been driven by [@pnbruckner]!
...
- **Repeat until**: Runs at least once, and decides after that to repeat as long as the condition(s) is/are met.
...
the chooser and the default is run instead.

### Automations & Scripts: Bonus! Cool down

An often requested feature is to allow for a cool down time on an automation.
What that entails is setting a limiting the run of a automation or script
...
seconds, prevent this automation to be ran more often than only once every 10
seconds. This is ideal for things like a doorbell.

## MDI icons updated

It has taken some time for us to upgrade to the newest version of Material
Design Icons, 5.3.45, there was a reason for that, version [5.0.45](https://dev.materialdesignicons.com/upgrade#4.9.95-to-5.0.45)
...
Most of the removed MDI icons can be found in [Simple icons](https://simpleicons.org/),
which is available as a [custom integration](https://github.com/vigonotion/hass-simpleicons).
