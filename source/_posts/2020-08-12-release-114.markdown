...
<p class='img'>
<img src='/images/integrations/script/script_modes.jpg' alt='Screenshot of lorem ipsum.'></a>
...
- [Lorem ipsum][lorem_ipsum docs], added by [@frenck]
...
- [Lorem ipsum][lorem_ipsum docs] has now support for Lorem ipsum, added by [@frenck]
...
- [Lorem ipsum][lorem_ipsum docs], done by [@frenck]
- [Lorem ipsum][lorem_ipsum docs], done by [@frenck]
...
With this change, a switch entity `switch.pi_hole` will be added if the user setup the integration with API key and default name (Pi-hole). Now, users can use `switch.turn_on` and `switch.turn_off` to enable and disable the Pi-hole service permanently. To disable the service for a given time, use `pi_hole.disable` with `entity_id: switch.pi_hole` and `duration: 00:00:15`. To target all Pi-hole services, use `entity_id: all`.
...
The `meteo_france` integration moved from a web scraping architecture to a API based architecture. With this major refactor the following change could break something:
  ...
  - `weather` platform is showing the current day forecast in first position of daily forecast (instead of tomorrow forecast)
...
When you have multiple Bond hubs, each with its own set of devices, those devices have IDs that the Bond integration uses to generate unique entity IDs in HA. Unfortunately, those IDs are only unique within the hub. So it is possible that unique IDs generated for entities from different hubs will not be unique and collide.

This has been fixed by prefixing device ID with a hub ID and the resulting ID is now truly unique.
After restarting you will see each bond entity twice. One which is live with the a unique ID. The second which is disabled with old unique ID.
