...
Also, when picking colors, make sure you pick a primary color, that it is
readable on both a light and dark background. For now this is just a local
setting and has to be applied on every device.
...
- If for any reason, corruption is detected in the Home Assistant recorder
  database (SQLite), the old database is moved and a new, fresh, database is
  created. This prevents issues during start up.
