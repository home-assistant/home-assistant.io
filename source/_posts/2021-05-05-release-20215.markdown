...
I would say this release is a bit more "slow" compared to the previous release
(which was epic, in my opinion!). But "slow" is not a bad thing!
...
(Spoiler alower: I've heard we might have a special guest, only Paulus didn't
...
cancel/react to the shutdown procedure better
...
## Tigger-based binary template sensor

In the previous release, we introduced a trigger-based template sensor. This
release extends on that features by adding support for trigger-based binary
sensors using templates.

This is an example of a webhook that is creating multiple template (binary)
sensors:
...
The above template would create 2 sensors, and one binary sensor based on
JSON data it receives via a webhook and could be triggered using this command:
...
So, webhooks is one example, but you can do tons of things with this magic!

Some other ideas:

- You could create a binary sensor from any event in Home
  Assistant event (e.g., a motion sensor event)
...
[VCN](https://github.com/codenotary/vcn) tool by running
`bash <(curl https://getvcn.codenotary.io -L)`.
