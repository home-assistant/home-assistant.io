...
any of their media players? And so the
...
Media Source is not limited to local media. Any integration can offer their
...
If you click it's media browser button, you will be presented with the available
local sources.
...
You can enter edit mode by pressing and holding the header of the sidebar, or
from your profile page. You can then drag the items in the order you want them
...
The logbook will now show what automation or script caused the the change and
...
In this example, a notification is send when one passes through two gates
...
So, Home Assistant now support the use of attributes in triggers & conditions.
...
A confirmation dialogue is shown before adding the suggested card.
</p>
...
- Slack notification now support change usernam/icon on the fly, which was
...
- The Netatmo integration was re-engineered and which reduced the number of API
  calls and also added webhook events to improve the overall responsiveness.
  It now supports controlling the outdoor camera flood light and got services
  to set the home/away status of occupants and the outdoor camera mode.
...
- Add default\_\* to device registry ([@balloob] - [#38829]) ([mikrotik docs])
...
- Drop UNIT\_ prefix for percentage constant ([@springstan] - [#39383])
...
- State condition can also accept an input\_\* Entity ID as state value ([@frenck] - [#39691])
