...
Also using special rows will not longer force you use the YAML editor, but you can still use the UI editor and edit the special rows. UI editors for the special rows will come in the future.
...
The person detail dialog has now a toggle that allows you to give the person acces to Home Assistant.
...
We now have our own media player for the Chromecast. It is build in the same app we use for showing Lovelace dashboards, this means that if you play media via Home Assistant and you where showing a Lovelace view, the Lovelace view will be restored after the media is finished playing!
