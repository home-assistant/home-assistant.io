...
So, lets face it: the previous release (0.110) was just jam packed with
...
Instead of waiting a couple of minutes before the Home Assistant frontend
...
to be ready. A result, not all devices and entities are available immediately.
...
as it is available, you can change or remove the configuration of an non-working
...
If an integration takes more time to set up, it will be shown in the logs.

While is is loading, every 60 seconds a log message will be shows that we are
awaiting the integration to be set up.

Another speed improvement is found in the way we load up integration itself.
Often integration have a basic setup and will load the various platforms
(like lights and switches) after that.

As of this release, Home Assistant will set up the integration and but will
no longer wait for the platforms finishing setting up. The individual platforms
will be finished in the background. Allowing the overall startup process to
continue and this results in a faster startup.
...
- [@gadgetmobile] when all out on the Blebox integration, adding support for
  a lot of platforms!
- Google Assistant now supports using an select helper (aka `input_select`), amazing work [@ZephireNZ]!
...
- **Insteon**
  ...
  - Full coexistance with the Insteon Hub app
