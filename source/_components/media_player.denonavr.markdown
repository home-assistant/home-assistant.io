...
- **show_all_sources** (*Optional*): If `true` all sources are displayed in sources list even if they are marked as deleted in the receiver. If `false` deleted sources are not displayed. `false` by default. Some receivers have a bug that marks all sources as deleted in the interface. In this case, this option could help.
