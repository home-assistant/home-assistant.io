...
- **name** (*Optional*): You can specify a name of the component, but do not have to. If you specify a name, the sensors will get an entity_id of 'sensor.[name]\_[monitored_condition], for example: 'sensor.lopik\_temperature' or 'sensor.weather\_groundtemperature'; 
  if no name is specified the sensors will be called sensor.br\_[monitored_condition], 
