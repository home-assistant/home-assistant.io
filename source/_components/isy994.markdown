...
tls:
  ...
  type: string
isy_variables:
  description: ISY Variables can be explicitly defined as as sensors, binary sensors, or switches.  See [Variables as Devices](#variables-as-devices) below.
  required: false
  type: dict
{% endconfiguration %}
...
All `isy994_control` events will have an `entity_id`, `control` and `value`parameter in its `event_data`. You'll need to refer to ISY994 documentation for the list of every possible control type, but the common ones are:
...
### Variables as Devices

ISY Variables can be explicitly defined in the configuration as a `sensor`, a `binary_sensor`, or a `switch`. The ISY Variable ID and Type (1-Integer or 2-State) must be provided, and optionally a unit of measurement, device class, and name. See the example config section below:
...
{% configuration %}
id:
  description: The ISY Variable ID number to use.
  required: true
  type: integer
type:
  description: The ISY Variable Type (1=Integer, 2=State).
  requried: true
  type: integer
unit_of_measurement:
...
device_class:
  ...
  type: string
payload_on:
  description: The payload that represents the on state (`binary_sensor` and `switch` only).
  required: false
  type: int
  default: 1
payload_off:
  description: The payload that represents the off state  (`binary_sensor` and `switch` only).
  required: false
  type: integer
  default: 0
{% endconfiguration %}
