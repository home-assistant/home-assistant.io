...
The easiest way to find your Siemens/LightwaveRF roller shutters is to add this to your `configuration.yaml`:
...
automatic_add:
  ...
  default: false
signal_repetitions:
  description: Because the RFXtrx device sends its actions via radio and from most receivers it's impossible to know if the signal was received or not. Therefore you can configure the roller shutter to try to send each signal repeatedly.
  required: false
  type: int
  default: 1
{% endconfiguration %}
