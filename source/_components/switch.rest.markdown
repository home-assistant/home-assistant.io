...
- **is_on_template** (*Optional*): A [template](/topics/templating/) that determines the state of the switch from the value returned by the GET request on the resource url. This template should compute to a boolean (True or False). Default is equivalent to `{{ value.json == body_on }}`. This means that by default, the state of the switch is on iff the response to the GET request matches `body_on`.
