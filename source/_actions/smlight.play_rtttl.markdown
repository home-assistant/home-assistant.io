...
The **Play RTTTL tone** action plays a <abbr title="Ring Tone Text Transfer Language">RTTTL</abbr> tone on the built-in buzzer of SMLIGHT Ultima devices. RTTTL is a compact text format for encoding simple melodies. If you want to learn more about the format, refer to [Ring Tone Text Transfer Language](https://en.wikipedia.org/wiki/Ring_Tone_Text_Transfer_Language). You can find and preview example tones using the [RTTTL Player](https://1j01.github.io/rtttl.js/).
