...
domain: telegram_bot
description: "Sends a photo through a Telegram bot to one or more chats."
---

Use this action to send a photo from your Telegram bot to one or more chats. You can point at a remote URL or a local file, and add a caption.

{% include actions/ui_header.md %}

To send a photo from an automation or a script:
...
{% options_ui %}
Notify target:
  description: "One or more Telegram notify entities to send the photo to. Each entity points at a specific bot and chat."
URL:
  description: "Remote path to an image."
File:
  description: "Local path to an image."
Caption:
  description: "The caption shown with the photo."
Verify SSL:
...
In YAML, refer to this action as `telegram_bot.send_photo`:
...
When the photo is sent, the action returns a `chats` list. Each item describes one delivered message:
