...
domain: telegram_bot
description: "Sends a document through a Telegram bot to one or more chats."
---

Use this action to send a document from your Telegram bot to one or more chats. You can point at a remote URL or a local file, and add a caption.

{% include actions/ui_header.md %}

To send a document from an automation or a script:

1. Go to {% my automations title="**Settings** > **Automations & scenes**" %}.
2. Open an existing automation or script, or select **Create automation** > **Create new automation**.
3. If you're setting up a new automation, add a trigger in the **When** section. Scripts don't need a trigger. They run when something else calls them.
4. In the **Then do** section, select **Add action**.
5. From the search box, search for and select **Telegram bot: Send document**.
6. Enter the **URL** or **File** of the document and, optionally, a **Caption** and other options.
7. Choose where to send it.
8. In the **Response variable** field, enter a name to store the data in, such as `document_message`.
9. Select **Save**.
...
{% options_ui %}
Notify target:
  description: "One or more Telegram notify entities to send the document to. Each entity points at a specific bot and chat."
URL:
  description: "Remote path to a document."
File:
  description: "Local path to a document."
Caption:
  description: "The caption shown with the document."
Verify SSL:
...
In YAML, refer to this action as `telegram_bot.send_document`:

{% example %}
action: |
  action: telegram_bot.send_document
  data:
    url: http://example.org/path/to/the/document.odf
    caption: My document
  response_variable: document_message
{% endexample %}
...
When the document is sent, the action returns a `chats` list. Each item describes one delivered message:
