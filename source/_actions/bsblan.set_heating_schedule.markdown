...
Use this action to set the weekly heating schedule for one BSB-LAN heating circuit. Select the heating-circuit sub-device to configure. Each day of the week can have one or more time slots during which heating is active. You only need to set the days you want to change.
...
To set the heating schedule from an automation or a script:
...
{% options_ui %}
Device:
  description: The BSB-LAN heating-circuit sub-device to configure.
  required: true
Monday time slots:
...
In YAML, refer to this action as `bsblan.set_heating_schedule`. Each day's slots are a list of time periods, where each period has a `start_time` and an `end_time`. A basic example looks like this:
...
device_id:
  description: >
    The BSB-LAN heating-circuit sub-device to configure.
  required: true
  type: string
monday_slots:
  description: >
