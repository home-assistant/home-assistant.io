...
{% options_ui %}
Trigger when:
  description: When multiple doors are targeted, controls when the trigger fires. Pick **Each** to fire every time any targeted door closes, **First** to fire only when the first targeted door closes, or **All** to fire only after every targeted door is closed. Default: **Each**.
  required: false
For at least:
