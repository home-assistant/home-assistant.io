...
Use **Relative humidity changed** to log humidity trends, trigger a fan when the air in a room becomes noticeably more humid, or alert you when a sensor reading shifts in a way that might signal a problem. Pair this trigger with [Relative humidity](/conditions/humidity.is_value/) to run the action only when the reading also meets a specific level at the moment the trigger fires.
