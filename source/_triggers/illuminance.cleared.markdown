...
Trigger when:
  ...
  required: false
For at least:
  description: How long the sensor or sensors must remain dark before the trigger fires. The default is `0` hours, `00` minutes and `00` seconds (fires immediately).
  required: false
{% endoptions_ui %}
