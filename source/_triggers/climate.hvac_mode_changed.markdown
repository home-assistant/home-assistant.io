...
Trigger when:
  description: |
    ...
    - **Each** (`any` in YAML, default): fire every time any targeted thermostat changes mode.
    - **First** (`first` in YAML): fire only on the first mode change.
    - **All** (`last` in YAML): fire only after every targeted thermostat changes mode.
For at least:
...
behavior:
  description: |
    ...
    - `any` (**Each** in the UI, default): fire every time any targeted thermostat changes mode.
    - `first` (**First** in the UI): fire only on the first mode change.
    - `last` (**All** in the UI): fire only after every targeted thermostat changes mode.
  required: false
  type: string
  default: any
for:
