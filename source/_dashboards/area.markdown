...
sensor_classes:
  ...
  description: A list of sensor device classes to display for the area. Most classes (such as temperature, humidity, pressure) show the median value when multiple sensors are present. Accumulative classes (power, energy, gas, water) show the sum instead.
features:
  required: false
