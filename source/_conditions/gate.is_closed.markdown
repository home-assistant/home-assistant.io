...
{% options_ui %}
Condition passes if:
  description: When multiple gates are targeted, controls how results combine. Pick **Each** to pass if at least one targeted gate is closed, or **All** to pass only when every targeted gate is closed.
For at least:
...
behavior:
  description: >
    When multiple gates are targeted, controls how results combine. Accepts `all` or `each`.
  required: false
  type: string
  default: each
for:
...
- With **Each**, the condition passes if at least one available targeted gate is closed.
- With **All**, the condition passes only if every available targeted gate is closed. If every targeted gate is `unavailable` or `unknown`, **All** passes and **Each** fails.
