...
{% options_ui %}
Condition passes if:
  description: When multiple vacuums are targeted, controls how results combine. Pick **Each** to pass if at least one targeted vacuum is docked, or **All** to pass only when every targeted vacuum is docked.
  required: true
For at least:
...
- With **Each** (default), the condition passes if at least one targeted vacuum is docked.
