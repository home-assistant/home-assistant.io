...
{% options_ui %}
Condition passes if:
  description: When multiple light sensors are targeted, controls how results combine. Pick **Each** to pass if at least one targeted sensor is detecting light, or **All** to pass only when every sensor is detecting light.
For at least:
...
behavior:
  description: |
    ...
    - `each` (default): passes if at least one targeted sensor is detecting light.
