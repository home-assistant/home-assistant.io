...
{% options_ui %}
Condition passes if:
  description: When multiple moisture sensors are targeted, controls how results combine. Pick **Each** to pass if at least one targeted sensor is detecting moisture, or **All** to pass only when every sensor is detecting moisture.
For at least:
...
behavior:
  description: |
    ...
    - `each` (**Each** in the UI, default): passes if at least one targeted sensor is detecting moisture.
