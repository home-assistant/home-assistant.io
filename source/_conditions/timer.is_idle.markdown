...
{% options_ui %}
Condition passes if:
  description: When multiple timers are targeted, controls how results combine. Pick **Each** to pass if at least one targeted timer is idle, or **All** to pass only if every targeted timer is idle.
For at least:
