...
{% options_ui %}
Condition passes if:
  description: When multiple windows are targeted, controls how results combine. Pick **Each** to pass if at least one targeted window is open, or **All** to pass only when every targeted window is open.
  required: true
For at least:
