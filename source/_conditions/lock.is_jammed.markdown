...
{% options_ui %}
Condition passes if:
  description: When multiple locks are targeted, controls how results combine. Pick **Each** to pass if at least one targeted lock is jammed, or **All** to pass only when every targeted lock is jammed.
  required: false
For at least:
