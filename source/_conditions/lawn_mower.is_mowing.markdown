...
{% options_ui %}
Condition passes if:
  description: When multiple lawn mowers are targeted, controls how results combine. Pick **Each** to pass if at least one targeted mower is mowing, or **All** to pass only when every targeted mower is mowing.
For at least:
