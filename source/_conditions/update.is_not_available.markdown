...
{% options_ui %}
Condition passes if:
  description: When multiple update entities are targeted, controls how results combine. Pick **Each** to pass if at least one targeted update is up to date, or **All** to pass only when every targeted update is up to date. The default is **Each**.
For at least:
