...
{% options_ui %}
Condition passes if:
  description: When multiple sensors are targeted, controls how results combine. Pick **Each** to pass if at least one targeted sensor detects gas, or **All** to pass only when every targeted sensor detects gas.
{% endoptions_ui %}
