...
{% options_ui %}
Condition passes if:
  description: When multiple alarm panels are targeted, controls how results combine. Pick **Each** to pass if at least one targeted alarm is armed away, or **All** to pass only when every targeted alarm is armed away.
For at least:
...
- Alarm panels that are unavailable (`unavailable`) or have an unknown state (`unknown`) do not count as armed away. With **Each** behavior, they are skipped. With **All** behavior, the condition fails if every targeted alarm is unavailable.
