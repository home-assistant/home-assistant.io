...
{% options_ui %}
Condition passes if:
  description: When multiple doors are targeted, controls how results combine. Pick **Each** to pass if at least one targeted door is closed, or **All** to pass only when every targeted door is closed.
For at least:
...
- With **Each**, the condition passes if at least one available targeted door is closed.
- With **All**, the condition passes only if every available targeted door is closed. If every targeted door is `unavailable` or `unknown`, **All** passes and **Each** fails.
