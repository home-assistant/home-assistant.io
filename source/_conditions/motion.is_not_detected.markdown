...
{% options_ui %}
Condition passes if:
  description: When multiple motion sensors are targeted, controls how results combine. Pick **Each** to pass if at least one targeted sensor is not detecting motion, or **All** to pass only when every sensor is not detecting motion.
For at least:
