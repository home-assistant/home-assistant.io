...
2. Install Home Assistant OS:
   ...
   - **For the virtual appliance images:** Load the appliance image into your virtual machine software. The appliance image initially has minimal free space and you should expand it to at least 32GB to avoid running out of space at runtime and during updates.  Home Assistant will automatically recognize and use the additional space the next time it starts.  An example command to resize the QCOW2 image is `qemu-img resize image.qcow2 +32G` (replace `image.qcow2` with the filename), and instructions for other image types are available via web search. You should assign RAM/CPU resources based on the guidance below and your anticipated add-on needs.
