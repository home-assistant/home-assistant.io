...
1. Download the appropriate install option:
   ...
   - As a virtual appliance (x86_64/UEFI image) for an virtual machine hypervisor:
...
2. Install Home Assistant:
   ...
   - **For virtualization using the virtual appliance images:** Load the virtual appliance image into your virtual machine (VM) hypervisor/virtualizer software. (Note: You are free to assign as much resources as you wish to the VM, please assign enough based on your add-on needs)
     - For VirtualBox (Oracle VM VirtualBox) create a new virtual machine, select "Other Linux (64Bit), assign it at least 2 GB of memory and "Use an existing virtual hard disk file", select the VDI file from above, afterwards edit the "Settings" of the VM and go "System" then Motherboard and Enable EFI, then "Network" "Adapter 1" Bridged and your adapter.
     ...
     - For KVM (Kernel-based Virtual Machine) on Linux create a new virtual machine in `virt-manager`, select "Import existing disk image", provide the path to the QCOW2 image above, choose "Generic Default" for the operating system, assign at least 2 GB memory and 1 vCPU, check the box for "Customize configuration before install" and select your bridge under "Network Selection", then under customization select "Overview" -> "Firmware" -> "UEFI x86_64: ...".
