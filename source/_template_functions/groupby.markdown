...
filter: |
  {% for state, entities in expand("group.home_lights") | groupby("state") %}
...
{% example %}
template: |
  {% for device_class, entities in expand("group.home_sensors")
...
{% example %}
template: |
  {% for area, entities in expand("group.home_lights")
