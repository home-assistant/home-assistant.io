...
If you are Windows users who is using [WinSCP](https://winscp.net/), please note that after running the installer, you will need to modify settings allowing you to "switch users" to edit your configuration files. The needed change within WinSCP is: **Environment** -> **SCP/Shell** -> **Shell** and set it to `sudo su -`. You may also have to change the SFTP server to avoid root permission errors that you will encounter when modifying other files on your Pi using WinSCP. Obtain the SFTP server by running `grep sftp /etc/ssh/sshd_config` on your Pi and you will get something like `Subsystem sftp /usr/lib/openssh/sftp-server`. Add the path in WinSCP under **Environment** -> **SFTP** and set it to `sudo su -c /usr/lib/openssh/sftp-server` (you may have to modify the path based on what was returned by the `grep` command. 
