...
This is done via setting the `hidden` attribute to `True`. This attribute is optional and defaults to `False`, so not setting it explicitly to `True` will show the automation entity in the main UI.
