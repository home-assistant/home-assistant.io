...
- Reolink products can not push motion events to a https adress (SSL), you need to configure a local http adress under `settings->system->network->Home Assistant URL->local network` in order for motion/people/visitor/vehicle detection to properly work.
