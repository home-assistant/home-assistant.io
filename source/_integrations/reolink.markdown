...
- There is a known issue with the firmware of Reolink cameras not supporting webhooks behind a HTTPS endpoint. Home Assistant instances that are only available to your camera over HTTPS (i.e. both the internal and external interface of Home Assistant are configured with certificates and use HTTPS) then certain webhooks will not work. For example, the Visitor binary sensor from the Video Doorbell will not be updated when the doorbell button is pressed.
