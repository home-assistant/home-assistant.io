...
- Do not set a a static IP in the Reolink device itself, but leave the **Connection Type** on **DHCP** under **Settings** -> **Network** -> **Network Information** -> **Set Up**. If you set is to **static** on the Reolink device itself, this is know to cause incorrect DHCP requests on the network causing Home Assistant to use the wrong IP address for the camera, resulting in connection issues. This originates from the Reolink firmware which keeps sending DCHP requests even when you set a static IP address in the Reolink device.
