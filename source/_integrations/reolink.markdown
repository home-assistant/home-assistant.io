...
The Reolink Home Assistant integration is supposed to only wake battery cameras once per 6 hours for a few seconds or perform a data update when the battery camera wakes by itself (at most once per hour). This should not have a big impact on battery life. You can check correct operation using the **Sleep status** entity. However, there are several factors that can have significant impact on battery life:
