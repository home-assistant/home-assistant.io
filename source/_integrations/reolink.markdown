...
- Reolink products can not push motion events to a HTTPS address (SSL), you need to configure a local HTTP address under `settings->system->network->Home Assistant URL->local network` in order for motion/people/visitor/vehicle detection to properly work.
