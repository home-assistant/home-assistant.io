...
This integration supports direct RS485 serial and TCP/IP-based transport connections to the inverter.
...
The direct RS485 connection can be made using a low-cost USB-RS485 converter. It works using a 2-wire interface but an interface with a separate ground reference may be more reliable. For TCP/IP-based connections, use an Ethernet-to-RS485 gateway that provides a raw TCP socket or serial bridge and can be configured to match the inverter's serial settings.
