...
4. **Test API key**: Verify your API key is correct by comparing it in the SABnzbd web interface
5. **Network connectivity**: If SABnzbd is on another device, ensure Home Assistant can reach it over the network
6. **Firewall settings**: Check that your firewall allows connections to SABnzbd's port
7. **Enable debug logging**: Temporarily enable [debug logging](/docs/configuration/troubleshooting/#debug-logs-and-diagnostics) for the SABnzbd integration to get more detailed error messages.
