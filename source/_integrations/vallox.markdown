...
| `duration`             | yes      | Duration to activate the profile for. `Integer` between 1 and 65535. Only applies to  `Boost`, `Fireplace` or `Extra` profiles. 65535 sets the profile without a timeout. | 