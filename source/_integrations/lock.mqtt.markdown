...
entity_picture:
  ...
  type: string
group:
  description: A list of unique IDs of the member lock entities. Set this if the lock entity represents a lock group. Note that the member entities must be already configured before the member entities will become visible in the UI at the moment a group entity is loaded.
  required: false
  type: list 
icon:
