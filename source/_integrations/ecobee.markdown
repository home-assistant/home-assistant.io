...
When enabling the Auxilary heat toggle, the ecobee thermostat HVAC mode will be changed to "Aux". Home Assistant will reflect that the thermostat is in "heat" mode, however. Disabling Auxilary heat will change the thermostat back to last active HVAC mode (heat, auto, etc).
