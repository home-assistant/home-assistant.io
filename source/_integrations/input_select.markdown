...
The preferred way to configure an input select is via the user interface at {% my helpers title="Settings -> Devices & Services -> Helpers" %}. Click the add button and then choose the [**Dropdown**](https://my.home-assistant.io/redirect/config_flow_start/?domain=input_select) option.
