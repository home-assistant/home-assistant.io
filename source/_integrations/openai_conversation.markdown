...
Service tier:
  description: Flex processing provides lower costs in exchange for slower response times (which might be useful for background automations) and [Priority processing](https://openai.com/api-priority-processing/) delivers significantly lower and more consistent latency compared to Standard processing for a higher price. See the [Pricing](https://developers.openai.com/api/docs/pricing) for details on the supported models. Please note that when the selected service tier is not available due to capacity or ramp rate limits, the request will be automatically processed as Standard tier (in this case, you will only be charged the Standard tier price).
Enable web search:
