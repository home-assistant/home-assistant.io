---
title: HEMS
description: Instructions on how to integrate ECHONET Lite devices using the HEMS integration.
ha_release: 2026.2
...
The **HEMS** (Home Energy Management System) {% term integration %} allows you to integrate ECHONET Lite compatible devices into Home Assistant. [ECHONET Lite](https://echonet.jp/) is a communication protocol standard widely used in Japan for smart home appliances, including air conditioners, solar power systems, storage batteries, and more.
...
The integration provides the following configuration options, which can be changed after setup via {% my integrations title="**Settings** > **Devices & services**" %} > **HEMS** > **Configure**:
...
The **HEMS** integration provides the following entity platforms based on device capabilities.

### Climate

For home air conditioners (Class code: 0x0130), the integration provides climate entities with:

- **HVAC modes**: Off, Auto, Cool, Heat, Dry, Fan only
- **Target temperature**: Set the desired temperature
- **Current temperature**: Room temperature sensor reading
- **Fan modes**: Auto, Low, High
- **Swing modes**: Off, Vertical, Horizontal, Both

### Binary sensors

Various detection and status sensors including:
...
- Humidifier/air purification status

### Sensors

Measurement sensors including:

- Temperature (room, outdoor, water)
- Humidity
- CO2 concentration
...
Multi-option settings including:
...
The **HEMS** integration uses both {% term polling %} and event-driven updates:
...
3. Try reloading the integration from {% my integrations title="**Settings** > **Devices & services**" %} > **HEMS** > {% icon "mdi:dots-vertical" %} > **Reload**.
...
3. The integration will create a repair issue if no frames are received for 5 minutes - follow the suggested resolution.
