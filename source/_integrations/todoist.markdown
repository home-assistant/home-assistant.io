...
- **reminder_location_direction** (*Optional*): Whether the reminder should be sent when entering or leaving the zone. Can only be set if `reminder_zone` or `reminder_location_name`, `reminder_latitude`, `reminder_longitude`, and `reminder_radius` are set.
