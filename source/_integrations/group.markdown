...
One can also create `binary_sensor`, `light` and `medio_player` groups which behave like a single entity. For `binary_sensor` the configuration should include the device class you want the group to appear as. 
```
binary_sensor:
    - platform: group
      name: Patio Doors
