...
- The group state is combined / calculated based on `type` selected to determine the minimum, maximum, latest (last), mean, median, range, product, sum or subtract of the collected states.
...
For `subtract` all entity values are subtracted from the first entity selected. Ensure you select the base value entity first.
...
all:
  ...
  default: false
type:
  description: "Only available for `sensor` group. The type of sensor: `min`, `max`, `last`, `mean`, `median`, `range`, `product`, `sum` or `subtract`."
  type: string
  required: true
ignore_non_numeric:
