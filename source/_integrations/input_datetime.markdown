...
The preferred way to configure input datetime is via the user interface at {% my helpers title="Settings -> Devices & Services -> Helpers" %}. Click the add button and then choose the [**Date and/or time**](https://my.home-assistant.io/redirect/config_flow_start/?domain=input_datetime) option.
