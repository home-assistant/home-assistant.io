...
Typically, the smart meter sends new data every 1 to 10 seconds. This option sets how often Home Assistant updates the entities (in seconds). If you set it to `0`, entities are updated each time data is received from the smart meter. Only entities with new data are published. If you set it to a value greater than `0`, power, current, voltage, frequency, and power factor readings are averaged over the selected interval, while the remaining readings publish the latest received value at that interval. This ensures the published values are representative of the whole interval. When using this mode, entities which stop receiving new data for a whole averaging interval are marked as `unavailable`.
