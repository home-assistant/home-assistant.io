...
Typically the smart meter sends new data every 5-10 seconds. This value defines the minimum time between entity updates in seconds. Setting this value to 0 will update all entities each time data is received from the smart meter. Setting it to a value higher than 0 will result in power, current, voltage and power factor readings being averaged over the chosen interval, while for the remaining readings the latest one is published at the chosen interval. This ensures that the user is provided with values representative of the chosen interval.
