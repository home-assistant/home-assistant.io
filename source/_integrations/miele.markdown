...
- **Operation state**:
  ...
  - **Descaling, degreasing, milk pipework cleaning cycles counter**: A set of sensors displaying the total number of cycles that the appliance has run. These sensors will be available only for devices supporting these maintenance programs (such as coffee machines or ovens with steam addition).
{% enddetails %}
