...
- `mode`: the demand control mode (`0` manual, `2` auto)
- `max_pow`: the maximum power limit as a percentage of the unit's nominal power (only presents if `mode` is `manual`)
