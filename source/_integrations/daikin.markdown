...
- The 'Today's total energy consumption' and 'Estimated power consumption' sensor is updated every time 100 Wh are consumed by all different operating modes summed together. This values are the values for the whole installation, not per device (unless, you have onle one device).
