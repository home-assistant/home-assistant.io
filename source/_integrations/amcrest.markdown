...
<div class='note'>
  The  amcrest integration reads the previous configurations into the configuration.yaml file.
...
events:
  description: >
    The lists of device events to be reported on the Home Assistant event bus.
    Events must be separated by commas.
    It is possible to use the keyword `All` to subscribe to all events.
    See example below.
  required: false
  type: string
  default: None
...
- The event option allow you to report the events generated by the device on the Home Assistant bus event. 
  The `event_type` will always be `amcrest.<eventname>`
  In this way it is easy to use device events directly from an automation.
  Events must be entered separated by commas.
  You can use the keyword `All` to intercept all events.
  Example:
```yaml
      All,RtspSessionDisconnect,VideoMotion,Invite
```
...
- alias: Test Doorbell
  trigger:
    platform: event
    event_type: amcrest.Invite
  action:
    service: persistent_notification.create
    data:
      message: "Someone push the doorbell button"
      title: "HEYOOOO"
```

- In the automation it is also possible to use the data deriving from the event.
  Using the keyword `trigger.event.data['data']['<nameofyourdataset>']`

{% raw %}

```yaml
#automations.yaml
- alias: Test RtspSessionDisconnect
  trigger:
    platform: event
    event_type: amcrest.RtspSessionDisconnect
  action:
    service: persistent_notification.create
    data_template:
      title: "new event RtspSessionDisconnect"
      message: "NEW event RtspSessionDisconnect payload Device:{{ trigger.event.data['data']['Device']}}" 
```
