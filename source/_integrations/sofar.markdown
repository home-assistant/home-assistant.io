...
- **PV strings**: Power for each solar panel string, plus voltage and current if you need the detail. Each string the inverter supports gets its own device, connected via the inverter. Only shown for PV-capable inverters.
- **Battery**: Voltage, current, power, temperature, state of charge, state of health, and charge cycles for each battery pack, plus combined power, state of charge, and state of health totals. Each pack gets its own device, connected via the inverter, and only packs that report back are added. A pack added later appears on its own, without reloading the integration. Only shown for inverters with battery storage.
