...
The meaning of `cool` or `heat` is that there is no schedule active, i.e. the system is manually set to in cooling- or heating-mode, the system will be active if the room temperature is above/below the thermostat setpoint.
