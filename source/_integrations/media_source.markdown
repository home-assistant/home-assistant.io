...
If you wish to make use of the `media-source://` URI for an action, and the media is already available
in the media browser (either as locally stored on the Home Assistant machine, or mapped using network storage)
the following steps can help to determine the `media-source` uri.
...
3) Copy the current URL from the URL bar.\
  *e.g: `https://home-assistant.local/media-browser/browser/app%2Cmedia-source%3A%2F%2Fmedia_source/%2Cmedia-source%3A%2F%2Fmedia_source%2Flocal%2FNAS_Media`*
