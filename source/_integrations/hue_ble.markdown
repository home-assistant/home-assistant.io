...
Hue lights connected using ZigBee still transmit on Bluetooth and will be discoverable by this intergration. It is possible to use both ZigBee and Bluetooth at the same time. This can be done by pairing the light to the ZigBee hub/switch and then using the Hue app to connect to the light over Bluetooth using the QR code and then using the voice assistant pairing feature to then pair the light to Home Assistant using this intergration.
