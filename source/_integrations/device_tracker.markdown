...
| `consider_home` | [uses platform setting]       | Seconds to wait until marking someone as **Not home** after being last seen. Allows you to override the global `consider_home` setting from the platform configuration on a per device level. The `consider_home` setting only applies for devices that were configured directly in YAML.                                                            |
