...
- Using an infrared remote adapter (proxy). An infrared remote adapter, such as the [Seeed Studio XIAO IR Mate](https://www.seeedstudio.com/XIAO-Smart-IR-Mate-p-6492.html), is a small ESPHome device with a built-in emitter that connects to Home Assistant over Wi-Fi (or Ethernet). You place it next to the IR-controlled device, and it sends IR commands over your network on behalf of Home Assistant, letting Home Assistant control IR-controlled devices anywhere in your home.

When you set up an ESPHome device configured as an infrared remote adapter, Home Assistant automatically discovers its built-in infrared emitters. Each emitter appears as an infrared entity, so you can select it when configuring an integration for an IR-controlled device.
...
- You select an infrared entity to control an IR-controlled device, which is separate hardware and not part of the infrared remote adapter.
