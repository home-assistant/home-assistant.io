...
The **Infrared** {% term integration %} acts as an abstraction layer between IR hardware (such as ESPHome) and device-specific integrations that send commands to, or receive signals from, IR devices.
...
- For an emitter, the state is the date and time of the last time an IR command was sent.
- For a receiver, the state is the date and time of the last time an IR signal was received.
