...
Constant API polling will prevent most Model S and Model X vehicles manufactured before 2021 from sleeping, so the Teslemetry integration will stop polling the API for 15 minutes, after 15 minutes of inactivity. You can call the `update_entity` service at any time to force polling the API, which will reset the timer.
