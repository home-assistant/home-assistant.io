...
1. Verify the configured hostname or IP address and port are correct.
2. If you use a hostname or SRV record, confirm that DNS resolves correctly.
3. Check firewall and port-forwarding rules if the server is on another network.
4. For Java Edition servers with release version 1.16+, confirm that the configuration parameter `enable-status` is set to `true` in the server configuration file (`server.properties`).
