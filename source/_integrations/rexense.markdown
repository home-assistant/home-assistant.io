...
### Sensors
For `REX‑METER` models, the following sensors are created:
...
### Switch
For `REX‑PLUG` models, a single **Switch** entity is created. It sends ON/OFF commands via WebSocket.

## Example Automation
```yaml
automation:
  - alias: "Turn on Rexense plug at sunset"
    trigger:
      platform: sun
      event: sunset
    action:
      ...
      target:
        entity_id: switch.rexense_meter_<device_id>_switch
```
