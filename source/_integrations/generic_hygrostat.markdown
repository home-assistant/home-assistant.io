...
min_cycle_duration:
  ...
  type: [time, integer]
dry_tolerance:
  description: Set a minimum amount of difference between the humidity read by the sensor specified in the *target_sensor* option and the target humidity that must change prior to being switched on. For example, if the target humidity is 45 and the tolerance is 2 the humidifier will start when the sensor equals or goes below 43.
  required: false
  default: 0.3
  type: float
wet_tolerance:
  description: Set a minimum amount of difference between the humidity read by the sensor specified in the *target_sensor* option and the target humidity that must change prior to being switched off. For example, if the target humidity is 45 and the tolerance is 2 the heater will stop when the sensor equals or goes above 47.
  required: false
  default: 0.3
  type: float
keep_alive:
