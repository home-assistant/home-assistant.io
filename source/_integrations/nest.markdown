...
If your thermostat supports a fan timer, the Fan trait exposes a timestamp sensor showing when the fan will turn off. This value represents a specific date and time rather than a countdown or remaining duration. The sensor state is `unknown` when the fan timer is inactive.
