...
- *Something went wrong, please try again in few minutes.*: According to Google's [Partner Connections Manager Error Reference](https://developers.google.com/nest/device-access/reference/errors/pcm) this error covers all other undocumented internal errors within Partner Connections. One of the issues that causes this error is synchronisation problems between the Nest and Google Home apps. Confirm that your Nest device is visible within both apps under the same Home. If it is missing within Google Home, create a new dummy home on the Nest app as this triggers the synchronisation process. (This is the workaround recommended by the Google support team). The dummy entry can be deleted once the Nest device is visible within the Google Home app.
