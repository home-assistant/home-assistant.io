...
- **WebRTC**: These devices support direct browser to camera communication and a super low latency stream. A [Picture Glance Card](https://www.home-assistant.io/lovelace/picture-glance/) can show the live stream in the grid with the *Camera View* set to `live` (not recommended for battery powered cameras). `camera` services like stream recording are *not supported*.
