...
The Fan trait exposes a timestamp sensor representing when the active fan timer is scheduled to turn off, which reports as `unknown` when the fan timer is inactive.
