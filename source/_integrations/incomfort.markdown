...
The hub does not have to be in the same network as HA, but must be reachable via port 80/HTTP. If the gateway is configured via DHCP, Home Assistant might be able to discovery it automatically. Once discovered, the host IP-address will be updated automatically when the dynamically assigned IP-address changes.
