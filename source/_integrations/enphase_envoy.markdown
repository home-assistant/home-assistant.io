...
### Lifetime value decreases with 1.2 MWh

Envoy Standard (not Metered), running firmware 8.2.4264, reportedly decreases the **Lifetime energy production** value with 1.2 MWh at irregular moments in time. The current hypothesis is that the step change occurs when one of the inverters exceeds a lifetime value of 1.2 MWh and resets to zero. This leads to the decrease with 1.2 MWh in the aggregated value for all inverters. It's not clear if this also happens for the metered Envoy.

{% details "History example for Envoy Lifetime energy production value irregular decreases" %}

Below example shows value decreases when multiple inverters reach 1.2 MWh lifetime values over a period of time. This would typically occur after some year(s) lifetime and then spread over some months based on individual solar panel production variations.
<figure>
  <img src="/images/integrations/enphase_envoy/enphase_envoy_production_decrease.png" alt="envoy lifetime decrease">
