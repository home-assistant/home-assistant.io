...
This action allows you to play music from the LMS music library. You can play an album, artist, genre, track, playlist or favorite. It has two forms. You can either search for an item using full text searching for the name of the item, or you can specify the item_id of the item, which is most likely retrieved previously using the search action. If you use the text search, your search must return a single result, which will be played or added to the current playlist.
