...
| Service Data Attribute 	| Required 	| Description                                                                                                                                      	|
|------------------------	|----------	|--------------------------------------------------------------------------------------------------------------------------------------------------	|
| `entity_id`            	| no        	| Entity (or list of entities) to set the configuration parameter on. An `entity_id` or `device_id` must be provided if not broadcasting the command.                       	|
| `device_id`            	| no        	| ID of device (or list of device IDs) to set the configuration parameter on. An `entity_id` or `device_id` must be provided if not broadcasting the command.                                         |
