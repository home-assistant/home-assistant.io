...
- **Delete:** Opens a dialog with options for removing the device. You can choose to remove it from the network using exclusion, remove a failed device from the adapter without excluding it from the network, or only delete it from Home Assistant.
