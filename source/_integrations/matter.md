...
- At this time there are only a few devices available that are compatible with the standard and some of them require you to join a beta/developer program.
- It is to be expected that more devices will hit the market during the 2nd quarter of 2023 and beyond.
...
- Platform support in Home Assistant is currently limited to switches, lights and (binary) sensors.
- Light platform is limited to on/off and brightness control only, support for Color and Color temperature control will be added soon.
...
These are the commission methods we currently support and have tested. There are other ways too, such as commission from other controllers (SmartThings, Alexa) which will probably work but these are untested.
...
- Make sure you are using the Google Chrome or Microsoft Edge browser.
- Open https://nabucasa.github.io/matter-example-apps/
- Attach the ESP32 device using an USB cable.
- Click the radiobutton next to the example you like to setup, in case of an M5 Stamp, click `Lightning app for M5STAMP C3`.
- Press `Connect`.
- In the popup dialog that appears choose the correct serial device, in most cases this will be something like "cu-usbserial" or alike.
- Click `Install Matter Lightning app example` and let it install the firmware on the device, this will take a few minutes.
- Once the device is flashed with the Matter firmware, connect to the device again but this time choose `Logs & console`.
- You are presented with a console interface where you see live logging of events. This is actually an interactive shell where you can type commands. For a list of all commands, type `matter help` and press enter.
- To commission the device, we need the QR code. In the console type in `matter onboardingcodes ble` and copy/paste the URL in your browser.
- Use the QR code to comission the device using one of the above instructions on your phone, e.g. using the Home Assistant Companion app.


### Eve Energy (power plug)
- Look for the Thread logo on the box to ensure you have the new device which is compatible with Matter.
- The Eve device runs on Homekit by default, you will need an iOS device to set it up out of the box.
- The Eve device uses Thread for communication, therefore you will need to have a Thread Border Router configured in your network setup, such as the Apple TV 4K (2021/2022), Homepod Mini, or Homepod V2.
- You need to join the [Eve Beta program here](https://www.evehome.com/en/meet-matter) and wait for instructions per email.
- Update the firmware of your Eve device using the Eve beta app to Matter.
- During the update process the Eve app will create a new QR code for you to save somewhere safe. This QR code is required when you want to commission the device to a Matter controller. The normal QR code attached to the device will no longer work!
- During the uprade priocess, the Eve app will join the device to Apple Home using Matter.
- Confirm that the device is working properly within Apple Home.
- Follow our instructions above to share the device from Apple Home to Home Assistant.
...
- Look for the M addition in the model namem ,a device without the M (regular P125) is not Matter compliant.
- This device is available in the US only.
- It runs Matter out of the box, so you can add it directly to the controller(s) of your choice!
