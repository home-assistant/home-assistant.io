...
| sensor | AirQualitySensor | All sensors that have `air_quality` as part of their `entity_id` or `air_quality` as their `device_class` will assume the sensor state is a valid HomeKit [`HMCharacteristicValueAirQuality`](https://developer.apple.com/documentation/homekit/hmcharacteristicvalueairquality) value and optionally accepts any of the following custom attributes [`nitrogen_dioxide_density`, `pm_density`, `pm_size`, `pm_2_5_density`, `pm_10_density`, `voc_density`] converting them to their respective characteristics and adding them to the HomeKit sensor. To maintain backwards compatibility with origional funcitonality, all sensors that have `pm25` as part of their `entity_id` or `pm25` as their `device_class` will create an AirQualitySensor with the `AirParticulateDensity` characteristic set using the sensor state and the `AirQuality` characteristic set based on a conversion of the sensor state to a HomeKit [`HMCharacteristicValueAirQuality`](https://developer.apple.com/documentation/homekit/hmcharacteristicvalueairquality) value. |
