            ...
            feature_list:
              ...
              keys:
                feature:
                  ...
                  type: string
            type:
              description: Only for `switch` and `fan` entities. Type of accessory to be created within HomeKit. Valid types for `switch` entities are `faucet`, `outlet`, `power_strip`, `shower`, `sprinkler`, `switch` and `valve`. Valid types for `fan` entities are `fan` and `air_purifier`.
              required: false
              type: string
              default: '`switch`'
            stream_count:
...
| switch                                                        | Switch                 | Represented as a switch by default but can be changed by using `type` within `entity_config`. Power strip switches (type `power_strip`) expose grouped switch members as independent outlets. Valve switches (type `faucet`, `shower`, `sprinkler`, or `valve`) can be linked with `linked_valve_duration` and `linked_valve_end_time`.                                                                                                                                                                                                                                                                                                                                                |
