...
Enable web search:
  description: The web search tool gives Claude direct access to real-time web content for [supported models](https://docs.anthropic.com/en/docs/agents-and-tools/tool-use/web-search-tool#supported-models), allowing it to provide up-to-date information beyond its knowledge cutoff. Claude automatically determines when to search based on your query, particularly for current events, recent information, or topics requiring real-time data. Queries with temporal indicators like "latest," "current," or "recent" will trigger immediate searches. For complex questions, Claude may perform multiple searches to gather comprehensive information, then synthesize the results with its existing knowledge into natural responses. Web searches add 2-10 seconds of latency per search. Complex queries requiring multiple searches will take longer to respond. Web search usage is [charged](https://docs.anthropic.com/en/docs/agents-and-tools/tool-use/web-search-tool#usage-and-pricing) separately from token usage.
Maximum web searches per conversation:
  description: This parameter limits how many searches Claude can perform in a single conversation. Once the limit is reached, Claude will notify you and continue the conversation using only its existing knowledge base—all other functionality remains available.
{% endconfiguration_basic %}
