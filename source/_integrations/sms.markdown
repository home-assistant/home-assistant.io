...
Note: if you have multiple USB devices, USB number order can change on boot, it's preferable to use your device in `/dev/serial/by-id/*` (ex: `/dev/serial/by-id/usb-HUAWEI_MOBILE_HUAWEI_MOBILE-if00-port0`)
