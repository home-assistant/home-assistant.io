...
**Please note:** the Telegram bot integration currently only supports one bot at a time. While adding additional bots won't throw an error, it will lead to unexpected behavior and potentially messages being sent to the wrong recipients.
