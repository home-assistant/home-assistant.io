...
{% configuration %}
address:
  description: Group address state or attribute updates will be sent to. GroupValueRead requests will be answered. `address` is the only valid configuration variable for types `time`, `date` and `datetime`.
  type: string
  required: true
type:
