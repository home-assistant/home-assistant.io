...
path or url:
  description: "Path to the OpenTherm Gateway device as supported by [PySerial](https://pythonhosted.org/pyserial/url_handlers.html). This is usually either a path to a serial device in `/dev/` if the gateway is connected via serial or USB or a URL in the form of `socket://[IP address]:[port]` if it is connected over the network.<br/>Examples: `/dev/ttyUSB0`, `socket://192.168.0.250:7686`". Default port for the current NodeMCU firmware is 25238 as it's mentioned in the [documentation](https://www.otgw.tclcode.com/otgwmcu.html). For older versions of the ESPHome firmware, you could choose port number in the device's settings via Web interface (e.g. `6638`). 
id:
...
<div class='note info'>
Once integration is set up, only `climate` entity is enabled. To avoid clutter in the state history, all sensors are disabled by default. If you're willing to use them, please navigate to the entities list and enable them. Not all the entities will be populated as different boilers support different features. 
</div>
