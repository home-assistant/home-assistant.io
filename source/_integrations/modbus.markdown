...
switches:
  ...
  keys:
    ...
    slave:
      ...
      default: 0
    verify:
      description: read from modbus device to verify switch. If used without attributes it used the toggle register configuration. If omitted no verify is done, but the state of the switch is set with each toggle.
      required: false
      type: map
      keys:
        address:
