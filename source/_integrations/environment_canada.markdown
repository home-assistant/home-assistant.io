...
{% important %}
Starting in Home Assistant 2026.X, the five separate alert sensors (`advisories`, `warnings`, `watches`, `statements`, and `endings`) have been replaced by a single `sensor.<name>_alerts` entity. If you have automations or dashboards that reference the old sensor entity IDs, you will need to update them.
{% endimportant %}

- **Alerts**: The total number of active alerts across all categories. The `alerts` attribute contains a list of all active alerts. Each alert includes the following fields:
  - `title`: Alert name.
  - `issued`: Publication datetime.
  - `color`: Risk color level (for example, `yellow` or `red`).
  - `expiry`: Expiration datetime.
  - `category`: Alert category (`advisories`, `warnings`, `watches`, `statements`, or `endings`).
  - `url`: Link to the full alert text.
  - `text`: Full alert text (when available).
  - `area`: Geographic area name (when available).
  - `status`: Alert status, for example `active` (when available).
  - `confidence`: Confidence level (when available).
  - `impact`: Impact description (when available).
  - `alert_code`: Alert code (when available).

Some fields are only available when the integration retrieves data from the GeoMet WFS API. They are omitted when the integration falls back to XML parsing.
...
To display a comma-separated list of all active alert titles:
...
- **`entity_id`** _(optional)_: The weather entity to get the forecast for.
...
- **`entity_id`** _(optional)_: The camera entity to set the radar type for.
- **`radar_type`** _(required)_: One of `Auto`, `Rain`, or `Snow`.
