...
`min` and `max` can also be used to reverse the direction of the device, if `min` is set to 100 and `max` is set to `0` device operation will be inverted. For example, when setting position to 40, a value of 60 will be sent to the device.
{% endnote %}
