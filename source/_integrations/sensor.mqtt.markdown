...
unit_of_measurement:
  ...
  type: string
value_template:
  description: "Defines a [template](/docs/configuration/templating/#processing-incoming-data) to extract the value. Within the template the following variables is available for the calculation: `entity_id`, the ID of the entity itself. The `entity_id` can be used to reference the entity's attributes."
  required: false
  type: template
{% endconfiguration %}
