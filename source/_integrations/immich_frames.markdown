...
The frame's options let you choose the portrait pairing mode, image orientation, rolling time range, portrait pairing window, output shape, photo fitting, and rotation interval. The output shapes are exact dimensions: 1280 × 800 landscape, 800 × 1280 portrait, or 720 × 720 square.
...
The image entity includes links to the displayed asset or assets in Immich when those links are available.

### Buttons

- **Next** selects and renders another photo.
- **Previous** returns to the previous rendered photo when history is available.
- **Refresh** requests an immediate update.
...
The integration provides sensors for the selected photo's date, location, people, tags, rating, camera, and favourite state. Lower-value metadata sensors are disabled by default and can be enabled from the entity registry when they are useful for a particular dashboard or automation.
...
The cache is tied to the Immich account, frame settings, output shape, and photo-fitting mode. A frame does not show an image from a different account or incompatible configuration. Authentication failures require the Immich integration to be repaired; ordinary network failures keep the last cached image visible and mark the frame as unavailable to the extent supported by the entity.
...
Photos can be cropped to fill the frame or shown in full with padding. Portrait photos can be displayed individually or paired side by side according to the selected mode and pairing window. If a preview cannot fill the configured output, the integration may request the full-size asset and falls back to the preview if the full-size request is unavailable.
...
Check the frame's **Screen shape** and **Photo fit** options. The rendered output uses the exact dimensions listed above; the selected fit determines whether the photo is cropped or padded.
