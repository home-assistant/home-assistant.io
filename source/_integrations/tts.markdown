...
* If you are using a SSL (eg `https://yourhost.example.org/...`) then you _must_ use the hostname in the certificate (eg `base_url: https://yourhost.example.org`). You cannot use an IP address since the certificate won't be valid for the IP address, and the cast device will refuse the connection.
