...
The **MQTT Switch** {% term integration %} lets you control your MQTT enabled switches.
...
To use an MQTT switch in your installation, [add an MQTT device as a subentry](/integrations/mqtt/#configuration), or add the following to your {% term "`configuration.yaml`" %} file.
...
entity_picture:
  ...
  type: string
group:
  description: A list of unique IDs of the member switch entities. Set this if the switch entity represents a switch group.
  required: false
  type: list
icon:
