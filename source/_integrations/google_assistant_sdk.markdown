...
If broadcasting doesn't work make sure: speakers aren't in do not disturb mode and that the Home Assistant server is in the same network as the speakers.

## Configuration

On the configure page, you can set the language code of the interactions with Google Assistant. If not configured, the integration picks one based on Home Assistant's configured language and country. Suported languages are listed [here](https://developers.google.com/assistant/sdk/reference/rpc/languages)
