...
* You have configured SSL/TLS for your Home Assistant. It doesn't need to be configured in Home Assistant though, e.g., you can be running [NGINX](/docs/ecosystem/nginx) in front of Home Assistant and this will still work. The certificate must be trustworthy (i.e., not self signed).
...
If you use [NGINX](/docs/ecosystem/nginx) as a proxy with authentication in front of your Home Assistant instance, you may have trouble with receiving events back to Home Assistant. It's because of authentication token that cannot be passed through the proxy.
