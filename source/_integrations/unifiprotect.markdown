...
Since UniFi Protect has its own release cycle, you should only upgrade UniFi Protect _after_ the next Home Assistant release comes out to ensure the new version is fully supported. Most importantly, that means **do not use Early Access versions of UniFi Protect if you want your UniFi Protect integration to be stable**. Early Access version can and will break the UniFi Protect Home Assistant in unexpected ways. If you really want to use Early Access version of UniFi Protect, you can disable automatic updates and wait for the next bugfix version of UniFi Protect to come out.

For example, the latest UniFi Protect Early Access version as of `2022.5.4` is UniFi Protect `2.0.0-beta.5`. So that means

* **do not** upgrade to `2.0.0-beta.7` until `2022.5.5` or `2022.5.6` comes out
