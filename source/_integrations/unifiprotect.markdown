...
* **Sensors** - Sensors are provided for data provided from the camera device. Sensors include "Is Dark", "Motion Detected", "Detected Object" (if camera has smart detections), and "Doorbell Chime" (if camera has a chime). Several diagnostics sensors are added including sensors on uptime, network connection stats, and storage stats. Doorbells will also have a "Voltage" sensor for troubleshooting electrical issues.
