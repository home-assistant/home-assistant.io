...
Any Zigbee device can only be connected to a single Zigbee Coordinator (i.e. only one Zigbee gateway). This is a limitation in the current (as well as previous) Zigbee protocol specifications, governed by the [CSA (Connectivity Standards Alliance)](https://csa-iot.org/all-solutions/zigbee/), as such it is a limit that applies to all Zigbee implementations and not just the ZHA implementation.
