...
To resolve issues with Zigbee devices that do not fully conform with the Zigbee standards the ZHA implementation relies on a library called "[ZHA Device Handlers](https://github.com/zigpy/zha-device-handlers). The few devices that deviate from the Zigbee specifications set by the [Zigbee Alliance](https://zigbeealliance.org) may therefore require proper bug reports with debug logs from users to assistant the developers in writting custom [ZHA Device Handlers](https://github.com/zigpy/zha-device-handlers/blob/dev/README.md) for all of a device functions to work properly with the ZHA integration in Home Assistant.
