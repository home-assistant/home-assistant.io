...
3. In the Scopes section, add the `chat:write` and `dnd:read` scopes, `Send messages as user`. If you get a `missing_scope` error when trying to send a message, check these permissions.
