...
- The assumed state is restored after a Home Assistant restart, but what comes back is what Home Assistant last assumed rather than anything read from the device. If the device was operated with its physical remote while Home Assistant was not running, the two are still out of sync afterwards.
