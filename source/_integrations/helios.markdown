...
The **Helios** {% term integration %} lets you control any Helios ventilation unit that is supported by the [helios_websocket_api](https://github.com/helios-ventilatoren-com/helios_websocket_api) (follow the link for a list of supported units).
...
For convenient switching of ventilation profiles in the GUI, just click on the `Helios` fan {% term entity %} to get a drop-down menu to select from. Alternatively, the action `fan/set_preset_mode` can be used.
