---
title: "Modelagem de Automação"
description: "Como usar modelos para criar automações mais poderosas."
---

A modelagem no Home Assistant permite que você crie automações e outras configurações que são mais dinâmicas e flexíveis. Os modelos são uma maneira poderosa de personalizar seu sistema Home Assistant.

## O que é um modelo?

Um modelo é um pedaço de texto que é avaliado pelo Home Assistant quando a automação é executada. O modelo pode conter espaços reservados que são substituídos por valores do estado atual do seu sistema.
