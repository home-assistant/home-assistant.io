...
3. Define the view settings:
   ...
   - If you want to see an icon, select the [view icon](#view-icon).
     - If an icon is defined, only the icon is shown. The text only is shown as a tooltip. However, setting the **Show icon with title** option allows to show both an icon and a text title.
